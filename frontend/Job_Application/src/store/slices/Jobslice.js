...
export const fetchJobs =
  ...
  async (dispatch) => {
    try {
      ...
      // Bug Fix: When city is "All", only include other filters
      if (city === "All") {
        queryParams = queryParams.filter((param) => !param.includes("city"));
      }
      ...
      // Bug Fix: When niche is "All", only include other filters
      if (niche === "All") {
        queryParams = queryParams.filter((param) => !param.includes("niche"));
      }

      link += queryParams.join("&");
      console.log("Fetching Jobs with URL:", link); // Debugging the final URL

      const response = await axios.get(link, { withCredentials: true });
      console.log("API Response Jobs:", response.data); // Log the response to inspect the data
...
export const fetchSingleJob = (jobId) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(jobSlice.actions.failureForSingleJob(error.response?.data?.message || "An error occurred"));
  }
};
...
export const postJob = (data) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(jobSlice.actions.failureForPostJob(error.response?.data?.message || "An error occurred"));
  }
};
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch(jobSlice.actions.failureForMyJobs(error.response?.data?.message || "An error occurred"));
  }
};
...
export const deleteJob = (id) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(jobSlice.actions.failureForDeleteJob(error.response?.data?.message || "An error occurred"));
  }
};
